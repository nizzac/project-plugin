...
namespace Nizzac\Project\Classes;
...
class ProjectApi
{
    ...
    {
        $url = config('nizzac.project::api.base_url');
        $token = config('nizzac.project::api.access_token');
        $this->api = Http::withToken($token)->baseUrl($url);
        $this->projectId = config('nizzac.project::api.project_id');
    }
